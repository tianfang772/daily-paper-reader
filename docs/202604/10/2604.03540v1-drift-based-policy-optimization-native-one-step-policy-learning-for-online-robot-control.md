...
tags: ["query:q1"]
score: 6.0
evidence: 用于在线机器人控制的一步生成式策略
tldr: 针对多步扩散策略在机器人控制中推理速度慢、难以满足高频闭环控制需求的问题，本文提出了一种原生单步生成式策略框架。通过引入基于漂移的策略（DBP）将迭代优化过程内化至模型参数中，并结合DBPO强化学习算法实现稳定的在线更新。该方法在保持多模态建模能力的同时，推理速度提升达100倍，支持百赫兹级别的实时机器人控制。
source: arxiv
...
result: 实验表明该方法在性能匹配或超越扩散策略的同时，推理速度提升100倍，并在真实双臂机器人上实现了105.2 Hz的高频稳定控制。
conclusion: 该框架成功解决了生成式策略在机器人控制中的效率瓶颈，为实现高效、多模态的在线机器人学习提供了有效的解决方案。
---

## 摘要
虽然多步生成式策略通过建模多模态动作分布在机器人操控中取得了卓越性能，但它们在推理时需要多步迭代去噪。因此，生成每个动作都需要数十到数百次网络函数评估（NFEs），这使得它们在高频闭环控制和在线强化学习（RL）中成本高昂。为了解决这一局限性，我们提出了一个原生单步生成式策略的两阶段框架，将细化过程从推理阶段转移到训练阶段。首先，我们引入了基于漂移的策略（DBP），它利用不动点漂移目标将迭代细化内化到模型参数中，从而在设计上构建出单步生成式骨干网络，同时保留了多模态动作建模能力。其次，我们开发了基于漂移的策略优化（DBPO），这是一个在线强化学习框架，为预训练的骨干网络配备了兼容的随机接口，在不牺牲单步部署特性的情况下实现了稳定的同策略更新。广泛的实验证明了所提框架在离线模仿学习、在线微调和真实世界控制场景中的有效性。DBP 在达到或超过多步扩散策略性能的同时，实现了高达 100 倍的推理加速。在具有挑战性的操控基准测试中，它也始终优于现有的单步基准模型。此外，DBPO 在在线设置中实现了有效且稳定的策略提升。在真实双臂机器人上的实验证明了 105.2 Hz 的可靠高频控制。
