...
title_zh: DynoSLAM：基于生成式图神经网络的真实世界社交导航动态 SLAM
authors: "Danil Tokhchukov, Veronika Morozova, Gonzalo Ferrer"
date: 2026-05-04
pdf: "https://arxiv.org/pdf/2605.02759v1"
tags: ["query:q1"]
score: 6.0
evidence: 用于运动预测和导航的随机世界模型
tldr: 针对传统SLAM在行人等动态环境中表现不佳的问题，本文提出DynoSLAM。该框架将社交感知图神经网络（GNN）集成到因子图优化中，通过随机世界模型和蒙特卡洛采样捕捉行人运动的多模态不确定性。这种紧耦合架构不仅提升了动态环境下的定位与跟踪精度，还为机器人导航提供了概率安全包络，实现了更稳健的社交导航。
source: arxiv
...
motivation: 传统SLAM算法依赖静态环境假设，难以处理行人等具有复杂交互行为和运动不确定性的动态实体。
method: 将生成式图神经网络与因子图优化紧耦合，利用蒙特卡洛采样建模行人运动的随机性，并通过马氏距离因子将其嵌入优化框架。
result: 实验证明该方法能有效防止确定性预测导致的优化失败，在保持高精度跟踪的同时为下游规划器提供概率安全包络。
conclusion: 该框架通过数学严谨的随机建模，为机器人在拥挤环境中的预测性避障和安全社交导航奠定了基础。
---

## 摘要
传统的同步定位与建图 (SLAM) 算法严重依赖静态环境假设，这极大地限制了它们在行人等移动实体密集的真实世界空间中的适用性。在这项工作中，我们提出了 DynoSLAM，这是一种紧耦合的动态图 SLAM (GraphSLAM) 架构，它将社交感知图神经网络 (GNN) 直接集成到因子图优化中。与使用僵化的等速启发式方法或确定性单智能体神经先验的传统方法不同，我们的框架将行人运动预测建模为一个随机世界模型 (World Model)。通过利用训练好的 GNN 进行蒙特卡洛展开，我们捕捉了人类交互的多模态认知不确定性，并通过动态马氏距离因子将其嵌入到 SLAM 图中。我们通过大量的仿真实验证明，这种随机公式化方法不仅保持了高精度的回顾性跟踪，还防止了由确定性“最大值 (argmax) 问题”引起的优化失败。最终，提取未来行人状态的经验均值和协方差矩阵，为下游局部规划器提供了一个数学严谨的概率安全包络，从而使机器人在密集拥挤的环境中能够实现具有预见性且无碰撞的导航。
