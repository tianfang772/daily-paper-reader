...
title_zh: 一种利用内部模型在下肢任务中实现模块化控制泛化的新型神经控制器
authors: "Munoz, D., Holland, D., Severini, G."
date: 2026-07-10
pdf: "https://www.biorxiv.org/content/10.1101/2023.12.04.569887v7.full.pdf"
tags: ["query:q2"]
score: 6.0
evidence: 用于下肢运动任务的内部模型
tldr: 本研究针对现有步态控制器生理基础薄弱且难以处理多种连续行为的问题，提出了一种基于内部模型的模块化神经控制器（IMMC）。该架构通过模拟中脑步行区激活不同的内部模型，将功能性肌肉协同组织成特定任务网络。该控制器在单一仿真中实现了站立、行走及倒走等多种任务的平滑切换与速度调节，其生物力学表现与实验数据一致，为解释人类复杂运动控制机制提供了新视角。
source: biorxiv
...
motivation: 现有的步态控制器缺乏生理学依据，且难以在单一仿真框架内实现多种运动行为之间的灵活切换。
method: 提出一种结合中脑步行区模型、内部模型和肌肉协同机制的模块化神经控制器架构。
result: 控制器成功实现了站立、前行、倒走等任务的平滑转换及步速调节，且仿真结果与人体实验数据高度吻合。
conclusion: 该模块化架构为产生异质性运动行为提供了一种具有生理合理性的神经控制机制。
---

## 摘要
神经控制的计算模型是评估运动控制原理的有力工具，而这些原理在传统的实验设置中往往无法直接测试。目前的步态控制器在生理基础方面存在局限，且仅能复制少量的离散行为。目标：在此，我们提出了一种新型神经控制器——基于内部模型的模块化控制器（Internal Model-based Modular Controller），它能够在单次模拟中实现多种下肢运动任务之间的切换。该架构包含一个中脑步行区（Mesencephalic Locomotor Region）的简化模型，用于激活不同的内部模型。这些内部模型将功能性肌肉协同作用组织成特定任务的网络，从而产生多块肌肉间的协调活动。结果：该控制器能够执行“站立-行走-站立”以及“站立-倒走-站立”的转换，并能通过调节少量控制信号在模拟中调制步态速度。观察到的模拟生物力学和肌肉激活模式与实验观察结果基本一致。结论：我们的结果表明，所提出的模块化架构为产生异质运动行为提供了一种合理的机制。影响声明：本研究提出了一种模块化神经控制器，该控制器利用内部模型和功能性肌肉协同作用，在单一模拟框架内重现了异质的腿部运动行为。
