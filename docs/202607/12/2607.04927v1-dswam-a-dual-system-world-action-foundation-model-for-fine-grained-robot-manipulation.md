...
tldr: 针对现有世界动作模型（WAM）在复杂多步任务中缺乏语言规划能力的问题，本文提出了DSWAM。该模型采用双系统架构：系统1作为WAM执行器负责底层控制，系统2作为视觉语言规划器负责将粗略指令分解为细粒度子任务。通过集成TensorRT加速和实时分块技术，DSWAM在保持物理感知能力的同时提升了执行效率，并在真实机器人变形物体操作任务中展现出优于传统VLA模型的性能。
source: arxiv
...
motivation: 现有的世界动作模型虽擅长物理执行，但缺乏将复杂指令分解为可执行子任务的显式语言规划能力。
method: 提出一种双系统架构，结合了负责子任务规划的视觉语言系统和负责动作生成的视频协同训练执行器，并引入实时分块技术优化推理。
result: 在DeMaVLA真实机器人评估中，DSWAM在相同数据和硬件条件下表现出比传统VLA策略更强的精细操作能力。
conclusion: DSWAM证明了结合语言规划与世界模型执行的架构能有效提升机器人处理复杂、细粒度操作任务的成功率与效率。
---
